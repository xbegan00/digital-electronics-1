...
In the receiver mode, the receiver is waiting for a start bit and then reads the incoming data and calculates parity. The incoming parity is shown on the RGB LED *LED16*, the calculated parity is shown on *LED17* The incoming data is displayed on the LED row *LED[15]* to *LED[0]*<br>
